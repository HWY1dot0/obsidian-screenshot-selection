...
Capture note content in Obsidian as a PNG. On desktop, selected rendered content is copied to the system clipboard. On mobile, selected text or the current markdown block is saved as an image file in your vault.
...
- Mobile output saves to `Attachments/Screenshots/` and inserts an image link when you run the command from the editor
...
3. Run **Screenshot selection or block to file**
4. The PNG is saved under `Attachments/Screenshots/`; when possible, an image embed is inserted below the captured block
...
- Mobile file capture can render selected markdown or the current markdown block from the editor
- Mobile clipboard image write is intentionally avoided because iOS and Android WebViews handle it inconsistently
